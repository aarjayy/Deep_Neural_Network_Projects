...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    next_h = np.tanh(b + np.dot(prev_h, Wh) + np.dot(x, Wx))
...
def rnn_step_backward(dnext_h, cache):
    ...
    h_deriv = (1 - next_h*next_h)*dnext_h
    db = np.sum(h_deriv, axis=0)
    dprev_h = np.dot(h_deriv, Wh.T)
    dWh = np.dot(prev_h.T, h_deriv)
    dx = np.dot(h_deriv, Wx.T)
    dWx = np.dot(x.T, h_deriv) 
    pass
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
      if i == 0:
        h[:,i,:], cache[str(i)] = rnn_step_forward(x[:,i,:], h0, Wx, Wh, b)
      else:
        h[:,i,:], cache[str(i)] = rnn_step_forward(x[:,i,:], h[:,i-1,:],Wx, Wh, b)
    pass
...
def rnn_backward(dh, cache):
    ...
    for i in range(T-1,-1,-1):
      dh_curr = dprev_h + dh[i]
      dx[i], dprev_h, dWx[i], dWh[i], db[i] = rnn_step_backward(dh_curr, cache[str(i)])
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    H = prev_h.shape[1]
    a = b + np.dot(prev_h, Wh) + np.dot(x, Wx)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    for i in range(4):
      dWx[:, i*H:(i+1)*H] = np.dot(x.T, da[i])
      dWh[:,i*H:(i+1)*H] = np.dot(prev_h.T, da[i])
      db[i*H:(i+1)*H] = np.sum(da[i], axis=0)
      dx += np.dot(da[i], Wx[:,i*H:(i+1)*H].T)
      dprev_h += np.dot(da[i], Wh[:,i*H:(i+1)*H].T) 
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
      if i == 0:
        h[:,i,:], c[:,i,:], cache_curr = lstm_step_forward(x[:,i,:],h0,c0,Wx,Wh,b)
        cache.append(cache_curr)
      else:
        h[:,i,:], c[:,i,:], cache_curr = lstm_step_forward(x[:,i,:],h[:,i-1,:],c[:,i-1,:],Wx,Wh,b)
        cache.append(cache_curr)
...
def lstm_backward(dh, cache):
    ...
    for i in range(T-1,-1,-1):
      dh_curr = dh_prev + dh[i]
      dc_curr = dc_prev
      dx[i], dh_prev, dc_prev, dWx[i], dWh[i], db[i] = lstm_step_backward(dh_curr, dc_curr, cache[i])
